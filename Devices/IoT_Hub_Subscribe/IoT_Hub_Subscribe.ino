...
static esp_err_t mqtt_event_handler(esp_mqtt_event_handle_t event)
{
  switch (event->event_id)
  {
    ...
    case MQTT_EVENT_DATA:
      ...
      if(isAlerting){
        if(String(incoming_data) == "lidClosed"){
          isAlerting=false;
          angle=0;
          myServo.write(angle);
          turnOffLight();
        ...
        if(String(incoming_data) == "lidOpen"){
          isAlerting=true;
          turnOnLightWithTheInitialPattern();
          rotate();
        }
      }
      break;
...
void turnOnLightWithTheInitialPattern(){
  sendswitch();
}

// turn off light when the ligth uses the initial pattern
void turnOffLight(){
  sendswitch();
  sendswitch();
  sendswitch();
...
{
if(isAlerting){
  Logger.Info("Alert!!!!You will be punished.");
}else{
  Logger.Info("No worries.You are Safe.");
}
