...
static String getBendSensorMessage(boolean isLidOpen){
  String messageBody;
  if(isLidOpen){
    messageBody ="{\"type\":\"lid\",\"isOpen\":true}";
  }else{
    messageBody ="{\"type\":\"lid\",\"isOpen\":false}";
  }
  return messageBody;
}

static String getCdSSensorMessage(boolean isLightOn){
  String messageBody;
  if(isLightOn){
    messageBody ="{\"type\":\"light\",\"isOn\":true}";
  }else{
    messageBody ="{\"type\":\"light\",\"isOn\":false}";
  }
  return messageBody;
}

static void sendTelemetry(String messageBody)
{
  ...
  String topic = "devices/your_device_name/messages/events/$.ct=application%2Fjson&$.ce=utf-8"
...
if(val<1650){
  if(!isLidOpen){
    ...
    String messageBody = getBendSensorMessage(isLidOpen);
    sendTelemetry(messageBody);
  }else{
    Serial.println("Being Open");
    digitalWrite(ledPin,HIGH);
  }
}else{
  if(isLidOpen){
    isLidOpen=false;
    Serial.println("The Lid Is Closed");
    String messageBody = getBendSensorMessage(isLidOpen);
    sendTelemetry(messageBody);
  }else{
    Serial.println("Being Closed");
    digitalWrite(ledPin,LOW);
  }
}
// value reference off:4095/on:2500
if(val2<3000){
  if(!isLightOn){
    isLightOn=true;
    Serial.println("The Light Is On");
    String messageBody = getCdSSensorMessage(isLightOn);
    sendTelemetry(messageBody);
  }else{
    Serial.println("Being Used");
  }
}else{
  if(isLightOn){
    isLightOn=false;
